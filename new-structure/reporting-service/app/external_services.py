...
logger = logging.getLogger("reporting-service.external")
...
async def fetch_sports(event_id: Optional[str], token: str = "") -> List[Dict[str, Any]]:
    ...
    return data if isinstance(data, list) else data.get("sports", data.get("sportsList", [])) or data


async def fetch_players(event_id: Optional[str], token: str = "") -> List[Dict[str, Any]]:
...
async def fetch_batches(event_id: str, token: str = "") -> List[Dict[str, Any]]:
