...
logger = logging.getLogger("organization-service.external")
...
async def get_event_year(
    ...
    return {"event_id": active_event.get("event_id"), "doc": active_event} if return_doc else active_event.get(
        "event_id"
    )


async def fetch_players(token: str = "") -> List[Dict[str, Any]]:
